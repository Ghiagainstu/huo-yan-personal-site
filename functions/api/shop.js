...
// 动态积分：掌握词×2 + 累计签到天×5 + 成功邀请×10 + 游戏星×1
// 注意：必须与 /api/me 的 score 口径完全一致（含 game_stars），否则前端显示的可用积分虚高，购买时误报"积分不够"
async function dynamicScore(db, userId) {
  ...
  return (mastered || 0) * 2 + (days || 0) * 5 + invitesUsed * 10 + gameStars * 1;
}

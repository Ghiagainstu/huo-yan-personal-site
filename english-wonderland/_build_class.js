...
.loadmore{display:block;margin:22px auto 0;padding:11px 26px;border:1px solid var(--line);background:#fff;color:var(--ink);border-radius:999px;font-size:14px;cursor:pointer;}
...
.modal-box{background:#fff;border-radius:14px;width:min(520px,92vw);max-height:86vh;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 20px 60px rgba(0,0,0,.3);}
.modal-h{display:flex;align-items:center;justify-content:space-between;padding:14px 18px;border-bottom:1px solid var(--line);font-size:15px;font-weight:600;}
.modal-h button{border:none;background:#f2f2f4;width:30px;height:30px;border-radius:50%;cursor:pointer;font-size:15px;color:var(--gray);}
.modal-h button:hover{background:#e5e5ea;}
.modal-ta{margin:14px 18px;border:1px solid var(--line);border-radius:10px;padding:10px 12px;font-size:14px;font-family:inherit;min-height:140px;resize:vertical;color:#333;line-height:1.6;outline:none;}
.modal-ta:focus{border-color:var(--gold);}
.modal-foot{display:flex;align-items:center;gap:10px;padding:10px 18px 16px;border-top:1px solid var(--line);}
.modal-foot button{margin-left:auto;border:none;background:var(--gold);color:#fff;padding:9px 22px;border-radius:10px;font-size:14px;cursor:pointer;}
.modal-foot .mtip{font-size:12px;color:var(--green);}
...
<div class="note">
  ...
  <b>逐词评价</b>：点每张卡下方的「✎ 写评价」按钮，弹出<b>单个评价框</b>（全页只此一个，不随卡片增多而卡顿），可写「图要重出 / 标签要改 / 归错类」等意见，<b>自动存浏览器本地（刷新不丢）</b>；上方「⬇️ 导出评价」下载 JSON 发我，或存为 <code>_vocab_comments.json</code> 让我重生成时嵌入；「⬆️ 导入评价」重新载入。列表默认只渲染 80 张，点「加载更多」继续翻看，避免一次载入全部卡片卡顿。
</div>
...
<button class="loadmore" id="loadMore" type="button">加载更多</button>
<div class="modal" id="modal"><div class="modal-box">
  <div class="modal-h"><span id="modalW"></span><button id="modalClose" type="button">✕</button></div>
  <textarea class="modal-ta" id="modalTa" placeholder="写评价…（图要重出/标签要改/归错类…）"></textarea>
  <div class="modal-foot"><span class="mtip" id="modalTip"></span><button id="modalSave" type="button">保存</button></div>
</div></div>
...
function makeCard(d){
  ...
  var has = (localComments[d.w]||'').trim();
  var cmt = '<button class="cmtbtn' + (has?' has':'') + '" data-act="cmt" data-w="' + d.w + '">' + (has?'✎ 已评':'✎ 写评价') + '</button>';
...
function computeList(){
  ...
  shown = 0;
  grid.innerHTML = '';
  if(currentList.length === 0){ grid.innerHTML = '<div style="color:#999;padding:30px;">无匹配</div>'; document.getElementById('loadMore').style.display='none'; return; }
  appendChunk();
}
function appendChunk(){
  var end = Math.min(shown + LIMIT, currentList.length);
  var frag = document.createDocumentFragment();
  for(var i=shown;i<end;i++){ frag.appendChild(makeCard(currentList[i])); }
  grid.appendChild(frag);
  shown = end;
  var more = document.getElementById('loadMore');
  if(shown < currentList.length){ more.style.display='block'; more.textContent = '加载更多（已显示 '+shown+' / '+currentList.length+'）'; }
...
grid.addEventListener('click', function(e){
  var t = e.target;
  if(t && t.classList && t.classList.contains('cmtbtn')){ openModal(t.getAttribute('data-w')); }
});
document.getElementById('loadMore').addEventListener('click', appendChunk);
// ---- 评价弹窗（全页仅 1 个 textarea，不随卡片膨胀）----
var modal = document.getElementById('modal');
var modalW = document.getElementById('modalW');
var modalTa = document.getElementById('modalTa');
var modalTip = document.getElementById('modalTip');
var activeW = null;
function openModal(w){
  activeW = w;
  modalW.textContent = w;
  modalTa.value = localComments[w] || '';
  modalTip.textContent = '';
  modal.classList.add('open');
  modalTa.focus();
}
function closeModal(){ modal.classList.remove('open'); activeW = null; }
document.getElementById('modalClose').addEventListener('click', closeModal);
modal.addEventListener('click', function(e){ if(e.target === modal) closeModal(); });
document.getElementById('modalSave').addEventListener('click', function(){
  if(!activeW) return;
  var v = modalTa.value;
  localComments[activeW] = v;
  saveLocal(activeW, v);
  var btn = grid.querySelector('.cmtbtn[data-w="'+activeW+'"]');
  if(btn){ var has = v.trim(); btn.classList.toggle('has', !!has); btn.textContent = has ? '✎ 已评' : '✎ 写评价'; }
  modalTip.textContent = '已保存「'+activeW+'」的评价（本地）';
...
fileInput.addEventListener('change', function(e){
  ...
  r.onload = function(){
    try{
      ...
      grid.querySelectorAll('.cmtbtn').forEach(function(btn){ var w=btn.getAttribute('data-w'); var has=(localComments[w]||'').trim(); btn.classList.toggle('has',!!has); btn.textContent= has?'✎ 已评':'✎ 写评价'; });
      tip('已导入 ' + n + ' 条评价');

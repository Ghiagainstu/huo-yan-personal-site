...
.card .cmtview{font-size:11px;margin-top:8px;color:#333;line-height:1.5;}
.card .cmtview .cmtbtn{display:inline-block;width:auto;margin-top:0;margin-left:6px;padding:2px 8px;}
.card .cmttxt{white-space:pre-wrap;word-break:break-word;}
...
var editingSet = {}; // 仅记录被点开编辑的词，避免 1620 个 textarea 常驻吃内存
...
function render(){
  ...
  for(var i=0;i<ALL.length;i++){
    ...
    var editing = !!editingSet[d.w];
    var cmt;
    if(editing){
      cmt = '<textarea class="cmt' + (cv?' has':'') + '" data-w="' + d.w + '" placeholder="写评价…（图要重出/标签要改/归错类…）">' + esc(cv) + '</textarea>';
    } else if(cv){
      cmt = '<div class="cmtview has" data-w="' + d.w + '"><span class="cmttxt">' + esc(cv) + '</span> <button class="cmtbtn" data-act="edit" data-w="' + d.w + '">✎改</button></div>';
    } else {
      cmt = '<button class="cmtbtn add" data-act="add" data-w="' + d.w + '">✎ 写评价</button>';
    }
...
grid.addEventListener('focusout', function(e){
  ...
  if(t && t.classList && t.classList.contains('cmt')){
    var w = t.getAttribute('data-w');
    if(!t.value.trim()){ delete editingSet[w]; render(); }
  }
});
